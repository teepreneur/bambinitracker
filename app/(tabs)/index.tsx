...
export default function HomeScreen() {
  ...
  useEffect(() => {
    const handleAppStateChange = (nextAppState: AppStateStatus) => {
      if (nextAppState === 'active') {
        ...
        if (today !== currentDate) {
          if (__DEV__) console.log(`[HomeScreen] Date changed from ${currentDate} to ${today}. Rolling over.`);
    ...
    const interval = setInterval(() => {
      const today = new Date().toISOString().split('T')[0];
      if (today !== currentDate) {
        if (__DEV__) console.log(`[HomeScreen] Midnight passed while app active! Rolling over.`);
        setCurrentDate(today);
      }
  ...
  // Track whether we've already triggered a sync for the current child+date combo
  const syncKeyRef = React.useRef<string | null>(null);

  // Auto-sync if we have fewer than 5 activities
  useEffect(() => {
    const syncKey = `${selectedChild?.id}_${currentDate}`;
    if (
      selectedChild?.id &&
      ageDays !== undefined &&
      !loadingActivities &&
      ...
      syncKeyRef.current = syncKey;
      if (__DEV__) console.log(`[HomeScreen] Triggering sync for ${selectedChild.name}. Current count: ${todayActivities.length}`);
      syncDaily.mutate({
